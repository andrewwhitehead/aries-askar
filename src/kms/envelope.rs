...
use crate::{
    crypto::{
        alg::{x25519::X25519KeyPair, KeyAlgorithm},
        buffer::SecretVec,
        encrypt::crypto_box::{
...
fn cast_x25519(key: &LocalKey) -> Result<&X25519KeyPair, Error> {
    if let Some(kp) = key.as_any().downcast_ref::<X25519KeyPair>() {
        Ok(kp)
    } else {
...
pub fn crypto_box(
    ...
    let mut buffer = SecretVec::from_slice_reserve(message, CBOX_TAG_LENGTH);
...
pub fn derive_key_ecdh_1pu(
    ...
    LocalKey::from_key_material(key_alg, derive)
}
